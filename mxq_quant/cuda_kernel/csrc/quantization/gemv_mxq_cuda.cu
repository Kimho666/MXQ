    ...
    const int IC, const int OC){
        ...
        for(int packed_group_idx = 0; packed_group_idx < 2; packed_group_idx++){
            ...
            *((float4*)(packed_weights)) = *((float4*)(weight + oc_idx * weight_w + packed_group_idx * (WARP_SIZE * 4) + threadIdx.x * 4));
            ...
            int inputs_ptr_delta = packed_group_idx * (2048/2) + threadIdx.x * (64/2); // 0, 2048/2, ...
            const double4* inputs_ptr = inputs + inputs_ptr_delta;

            #pragma unroll
            for (int ii = 0; ii < 3; ii++){ // Get first 3 uint32 packed weights(total 3*16*2b)
                ...
                *((double4*)packed_inputs) = *(inputs_ptr + ii * 16);
            ...
            // calculate 16 4b weight and activations
            {
                ...
                half packed_inputs[16];
                *((double4*)packed_inputs) = *(inputs_ptr + 3 * 16);
