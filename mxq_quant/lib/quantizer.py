...
def quantize_dequantize_new(x, scale, zero, maxq, eps=1e-9):
    q = torch.round(torch.round((x - zero) / scale.clamp_min(eps) * maxq).div(maxq))
    return q * scale.clamp_min(eps) + zero
...
class Quantizer(nn.Module):
    ...
    def find_params(self, x, weight=False):
        ...
        if self.bits==1:
            #self.scale = (xmax.abs() + xmin.abs()) / 2
            self.scale = xmean
    ...
    def quantize_dequantize(self, x):
        if self.bits==1:
            return self.quantize_dequantize_1b(x)
        else:
            if self.ready():
                return quantize_dequantize(x, self.scale, self.zero, self.maxq)
            return x
    
    def quantize_dequantize_1b(self, x):
        if self.ready():
            x_pos_mask = (x >= 0)
            x_neg_mask = (x < 0)
            xx = self.scale * x_pos_mask.float() + (-self.scale) * x_neg_mask
            return xx
        return x
